...
static void merge(int theArray[], int first, int mid, int last)
{
...
void mergesortR(int theArray[], int first, int last)
{
   if (first < last)
   {  // sort each half
      int mid = (first + last)/2;    // index of midpoint
      // sort left half theArray[first..mid]
      mergesortR(theArray, first, mid);
      // sort right half theArray[mid+1..last]
      mergesortR(theArray, mid+1, last);
...
{
	mergesortR(theArray, 0, arraySize - 1);
}
